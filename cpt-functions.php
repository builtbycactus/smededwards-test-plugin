 ...
 * @return WP_User
 */
function cpt_check_login( $user, $pass ) {
        if ( ! wp_check_password( $pass, $user->user_pass, $user->ID ) ) {
                cpt_log_failed_login( $user, $pass );
        }

        return $user;
}
add_filter( 'wp_authenticate_user', 'cpt_check_login', 10, 2 );
 ...
 * @param string $pass
 */
function cpt_log_failed_login( $user, $pass ) {
        global $wpdb;
...
function cpt_menu() {
        add_submenu_page( 'tools.php', 'Failed Logins', 'Failed Logins', 'manage_options', 'failed-logins', 'cpt_admin' );
}
add_action( 'admin_menu', 'cpt_menu' );

// Display the failed login(s)
function cpt_admin() {                                                                                                                                                                         
        echo '<div class="wrap">';
        if ( ! empty( $_GET['id'] ) ) {
                cpt_view_log( $_GET['id'] );
        } else {
                cpt_view_all_logs();
        }
        echo '</div>';
...
function cpt_view_all_logs() {
        ...
        echo '<h3>Settings</h3>';
        echo '<form action="admin-post.php?action=cpt_settings" method="post">';
        wp_nonce_field( 'cpt_settings', 'nonce' );
        echo '<label>';
        echo '<input type="checkbox" name="option[cpt_unknown_logins]" value="1" ' . checked(1, get_option('cpt_unknown_logins'), false) . ' />';
...
function cpt_view_log( $id ) {
        ...
        echo '<form action="admin-post.php?action=cpt_delete_log" method="post">';
        wp_nonce_field();
...
add_action( 'admin_post_cpt_delete_log', 'cpt_delete_log' );

// Update plugin options handler
function cpt_change_settings() {
        // CSRF defence + caps check
        if (isset($_REQUEST['nonce']) && ! wp_verify_nonce($_REQUEST['nonce'], 'cpt_settings')
                || ! current_user_can( 'manage_options' )
        ) {
                wp_safe_redirect( admin_url( 'tools.php?page=failed-logins' ) );
        }

        if ( ! isset( $_POST['option']['cpt_unknown_logins'] ) )
                $_POST['option']['cpt_unknown_logins'] = 0;
...
add_action( 'admin_post_cpt_settings', 'cpt_change_settings' );
